...
namespace OnlineLearningManagementSystemProject.Services
{
    public class UploadStaffFileRepository : IUploadStaffFileRepository
    {
        ...
        public IEnumerable<Material> GetLecturesByMaterialDoctor(int corseid, int staffid)
        {
            return context.Materials
               .Include(m => m.Staff)
               .Include(m => m.Course)
               .Where(m => m.StaffId == staffid && m.CourseID == corseid);
        }

        public int UploadDocument(IFormFile formFile, [Bind("MaterialName,CourseID,StaffId")] Material material)
        {
